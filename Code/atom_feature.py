...
aa_dict = {
    ...
    'x': '[NH2:1]C[C:1](=[O:1])[OH:1]'  # X用甘氨酸(G)作为默认值
}
...
pre_known_charges = {
    ...
    'y': [-0.31816209743565643, 0.10718801610357671, -0.0017850394584956058, -0.045610041081834043,
          ...
          -0.48007770020325236],
    'x': [-0.32100388796548518, 0.091814239632494035, 0.31675597840558584, -0.25094762709006846, -0.48029713002439572]  # X用甘氨酸(G)的电荷
}
...
def convert_to_graph_channel(seq: str, max_seq_len=50):
    ...
    maxNumAtoms = 15
    graphFeaturesOneSeq = []
    # 对于未知氨基酸，使用甘氨酸(g)作为默认值
    seqSMILES = [aa_dict.get(b, aa_dict['g']) for b in seq]
    ...
    for i_, aminoAcidSMILES in enumerate(seqSMILES):
        ...
        if numAtoms <= maxNumAtoms:
            ...
            # 对于未知氨基酸，使用甘氨酸(g)的电荷作为默认值
            charges = pre_known_charges.get(seq[i_], pre_known_charges['g'])
            for j_ in range(numAtoms):
                paddedNodeFeatureNorm[j_, -1] = charges[j_] if j_ < len(charges) else 0.0
            ...
            for k in range(numAtoms):
                graphFeature[k, -1] = charges[k] if k < len(charges) else 0.0
