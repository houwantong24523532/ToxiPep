...
# ============== 消融评估: Without Structure Encoder ==============
print("=" * 60)
print("消融评估: Without Structure Encoder (仅 Sequence Encoder)")
print("=" * 60)
...
# 加载消融模型
model = ToxiPep_WithoutStructureEncoder(vocab_size, d_model, d_ff, n_layers, n_heads, max_len).to(device)

checkpoint = torch.load("best_model_without_structure_encoder.pth", map_location=device, weights_only=False)

if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
    model.load_state_dict(checkpoint['model_state_dict'])
    optimal_threshold = checkpoint.get('optimal_threshold', 0.5)
    print(f"已加载模型: best_model_without_structure_encoder.pth (最优阈值: {optimal_threshold:.2f})")
else:
    model.load_state_dict(checkpoint)
    optimal_threshold = 0.5
    print("已加载模型: best_model_without_structure_encoder.pth (使用默认阈值: 0.5)")
...
print("\n" + "=" * 40)
print("测试集评估结果 (Without Structure Encoder)")
print("=" * 40)
...
save_metrics_to_csv(metrics, "test_metrics_without_structure_encoder.csv")
