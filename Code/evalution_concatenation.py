...
model = ToxiPep_Concatenation(vocab_size, d_model, d_ff, n_layers, n_heads, max_len,
                              structural_config=structural_config).to(device)
checkpoint = torch.load("best_model_concatenation.pth", map_location=device, weights_only=False)
...
save_metrics_to_csv(metrics, "test_metrics_concatenation.csv")
